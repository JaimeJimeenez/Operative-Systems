...
static int my_unlink(const char* path) {
    ...
    //Se encuentra el número de entrada de directorio a ese fichero. Tabla asocaida a los inodos
    fprintf(stderr, "--->>> path %s", path);
    int indexNode = findFileByName(&myFileSystem, (char*) path);

    // Se encuentra el indice del inodo asociado a ese fichero para posteriormente acceder al inodo
    int iNodeIndex = myFileSystem.directory.files[indexNode].nodeIdx;
    NodeStruct* inode = myFileSystem.nodes[iNodeIndex];

    //Se trunca el tamaño del fichero con resizeNode
    resizeNode(iNodeIndex, 0);

    //Se marca como libre el fichero  
    myFileSystem.directory.files[indexNode].freeFile = true;
    
    //Se actualizan el número de ficheros existentes en el directorio y el número de inodos libres.
    myFileSystem.directory.numFiles--;
    myFileSystem.numFreeNodes++;

    //Se actualizan los datos del inodo en cuestión
    inode->freeNode = true;
    inode->numBlocks = 0;
    inode->fileSize = 0;
    
    //Se actualiza el sistema de ficheros en el disco
    updateDirectory(&myFileSystem);

    //Se actualizan los inodos
    updateNode(&myFileSystem, iNodeIndex, inode);
    free(inode);
    updateBitmap(&myFileSystem);
...
static int my_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi) {
    ...
    //Comprobar si el desplazamiento es correcto
    if (offset >= node->fileSize)
        return 0;
    
    //Saber el numero de bytes que se quieren leer
    ...
    while (totalRead < bytes2Read) {
        ...
        int blockOffset = offset % BLOCK_SIZE_BYTES;

        //Se lee el bloque y se almacena en el buffer
        if (readBlock(&myFileSystem, currentBlock, &buffer) == -1) 
            return -EIO;

        //Para cada bloque se lee el contenido del buffer que se ha devuelto por readBlock() y se almacena en el buffer
        int i;
        for (i = blockOffset; i < BLOCK_SIZE_BYTES && totalRead < bytes2Read; i++)
